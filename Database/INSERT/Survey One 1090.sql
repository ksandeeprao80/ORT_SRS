...
BEGIN TRAN
	...
	SELECT 
		...
		1 AS CategoryId, 1 AS LanguageId, GETDATE()+15
...
-- 	SELECT @SurveyId,SettingId,1,'True' FROM MS_SurveySettings 
	...
	SELECT 
		...
		CASE WHEN SettingName = 'ANSWER_FONT' THEN 'Verdana'
			WHEN SettingName = 'ANSWER_FONT_COLOR' THEN '#5C3658'
			WHEN SettingName = 'ANSWER_FONT_SIZE' THEN '12px'
			WHEN SettingName = 'HEADER_FONT' THEN 'Verdana'
			WHEN SettingName = 'HEADER_FONT_SIZE' THEN '20px'
			WHEN SettingName = 'HEADER_FONT_COLOR' THEN '#D2691E'  
			WHEN SettingName = 'QUESTION_FONT' THEN 'Verdana'
			WHEN SettingName = 'QUESTION_FONT_COLOR' THEN '#CBFEF8' 
	...
	--SELECT * FROM TR_SurveySettings WHERE SurveyId = 1090  AND SettingId = 33
	-- UPDATE TR_SurveySettings SET Value = '#5C3658' WHERE SurveyId = 1090 AND SettingId = 9 -- ans
	-- UPDATE TR_SurveySettings SET Value = '#D2691E' WHERE SurveyId = 1090 AND SettingId = 33 -- hea
	-- UPDATE TR_SurveySettings SET Value = '#CBFEF8' WHERE SurveyId = 1090 AND SettingId = 24 -- que
	...
	INSERT INTO TR_MediaInfo -- Auto Advance True
