...
BEGIN TRAN
	...
	SELECT 
		'Latas Music Survey' AS SurveyName, 1 AS CustomerId, 1 AS StarMarked, 1 AS RewardEnabled,
		1 AS CreatedBy, GETDATE() AS CreatedDate, 1 AS InActive, 1 AS StatusId,
		1 AS CategoryId, 1 AS LanguageId, GETDATE()+20
...
-- 	SELECT @SurveyId,SettingId,1,'True' FROM MS_SurveySettings 
	...
	INSERT INTO TR_SurveySettings -- Back Button True
	(SurveyId, SettingId,CustomerId,Value)
	SELECT @SurveyId,SettingId,1,'True' FROM MS_SurveySettings WHERE SettingName NOT LIKE '%FONT%'
	...
	SELECT 
		...
		CASE WHEN SettingName = 'ANSWER_FONT' THEN 'Arial'
			WHEN SettingName = 'ANSWER_FONT_COLOR' THEN '#929E47'
			WHEN SettingName = 'ANSWER_FONT_SIZE' THEN '14px'
			WHEN SettingName = 'HEADER_FONT' THEN 'Arial'
			WHEN SettingName = 'HEADER_FONT_SIZE' THEN '22px'
			WHEN SettingName = 'HEADER_FONT_COLOR' THEN '#F0F7A8' 
			WHEN SettingName = 'QUESTION_FONT' THEN 'Arial'
			WHEN SettingName = 'QUESTION_FONT_COLOR' THEN '#C7E4BA'
			WHEN SettingName = 'QUESTION_FONT_SIZE' THEN '16px' END AS Value
	FROM MS_SurveySettings WHERE SettingName LIKE '%FONT%'

	--SELECT * FROM MS_SurveySettings WHERE SettingName LIKE '%FONT%'
	--SELECT * FROM TR_SurveySettings WHERE SurveyId = 1091
	-- UPDATE TR_SurveySettings SET Value = '#929E47' WHERE SurveyId = 1091 AND SettingId = 9 -- ans
	-- UPDATE TR_SurveySettings SET Value = '#F0F7A8' WHERE SurveyId = 1091 AND SettingId = 33 -- hea
	-- UPDATE TR_SurveySettings SET Value = '#C7E4BA' WHERE SurveyId = 1091 AND SettingId = 24 -- que
	...
	INSERT INTO TR_MediaInfo -- AutoAdvance True
	(QuestionId,FileLibId,CustomerId,Randomize,AutoAdvance,ShowTitle,Autoplay,HideForSeconds)
	SELECT @QuestionId3,@FileLibId1,1,1,1,1,1,5 UNION
	SELECT @QuestionId4,@FileLibId2,1,1,1,1,1,5 UNION
	SELECT @QuestionId5,@FileLibId3,1,1,1,1,1,5 UNION
	SELECT @QuestionId6,@FileLibId4,1,1,1,1,1,5 UNION
	SELECT @QuestionId7,@FileLibId5,1,1,1,1,1,5
