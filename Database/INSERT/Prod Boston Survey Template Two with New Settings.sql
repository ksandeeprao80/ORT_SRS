...
BEGIN TRAN
	...
	SELECT 
		'Boston Prod Quota Survey' AS SurveyName, 1 AS CustomerId, 1 AS StarMarked, 1 AS RewardEnabled,
...
--------------------------------------------------------------------
	...
	SELECT @ImageDisplay, SettingId, 
			CASE WHEN SettingId IN(1,3,6) THEN 'True'  
				...
				WHEN SettingId IN(8) THEN '1436' -- '353'--ORT 
				END AS Value  
	FROM MS_QuestionSettings
	
	--SELECT * FROM UPDATE TR_QuestionSettings SET Value = '1410' WHERE SettingId = 8 AND Value = '1436'
...
--------------------------------------------------------------------
	...
	INSERT INTO TR_QuestionSettings(QuestionId,SettingId,Value)
	SELECT @4, SettingId, 
			CASE WHEN SettingId IN(1,2,3,10,11,16) THEN 'True'  
				WHEN SettingId IN(4,5,6,13,14) THEN 'False'
				WHEN SettingId IN(7) THEN '0'
	...
	---- If Setting Id 16 is True then entry should be in TR_QuestionQuota
	--INSERT INTO DBO.TR_QuestionQuota
	--(QuestionId, LogicExpression,FalseAction)
	--SELECT @4,'Count(AnswerId('+CONVERT(VARCHAR(12),@Male)+')) == 3',NULL  
	--INSERT INTO DBO.TR_QuestionQuota
	--(QuestionId, LogicExpression,FalseAction)
	--SELECT @4,'Count(AnswerId('+CONVERT(VARCHAR(12),@Female)+')) == 4',NULL  
...
--------------------------------------------------------------------
	...
	---------------------------
	-- Question for MALE and FEMALE
	-- If Setting Id 16 is True then entry should be in TR_QuestionQuota
	INSERT INTO DBO.TR_QuestionQuota
	(QuestionId, LogicExpression,FalseAction)
	SELECT @4,'Count(AnswerId('+CONVERT(VARCHAR(12),@Male)+')) == 3',@EndOfBlock  
	INSERT INTO DBO.TR_QuestionQuota
	(QuestionId, LogicExpression,FalseAction)
	SELECT @4,'Count(AnswerId('+CONVERT(VARCHAR(12),@Female)+')) == 4',NULL  
	...
	-- select 
	/*
	SELECT MT.TemplateId,mti.ItemName,MTI.ItemId,MA.AttributeName, TT.Value
	FROM DBO.TR_Templates TT
	INNER JOIN DBO.MS_Templates MT
		ON TT.TemplateId = MT.TemplateId 
		AND MT.IsActive = 1
		AND TT.TemplateId = 2--ISNULL(@TemplateId,TT.TemplateId)
	...
	*/
	...
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'#093f78' FROM MS_SurveySettings WHERE SettingName = 'HEADER_FONT_COLOR'
	...
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'#000' FROM MS_SurveySettings WHERE SettingName = 'ANSWER_FONT_COLOR'
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'14px' FROM MS_SurveySettings WHERE SettingName = 'ANSWER_FONT_SIZE'
	...
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'#000' FROM MS_SurveySettings WHERE SettingName = 'QUESTION_FONT_COLOR'
	...
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'10' FROM MS_SurveySettings WHERE SettingName = 'QuotaCount' 
	 --Quota Email
	...
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'False' FROM MS_SurveySettings WHERE SettingName = 'VerifySkipLogic'
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'2' FROM MS_SurveySettings WHERE SettingName = 'TemplateId'

	--- END OF SURVEY SETTINGS --------------------------------------------------------------------------------------

	EXEC UspPublishSurvey @SurveyId
