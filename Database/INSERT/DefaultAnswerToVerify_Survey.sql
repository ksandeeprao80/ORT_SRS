...
--------------------------------------------------------------------
	...
	INSERT INTO TR_QuestionSettings(QuestionId,SettingId,Value)
	SELECT @Q7, SettingId, 
			CASE WHEN SettingId IN(1,2,3,13,14) THEN 'True'  
	...
	--------------------------------------------------------------------
		-- Q7
	DECLARE @DefaultAns INT
	SET @DefaultAns = 0
	
	INSERT INTO DBO.TR_SurveyQuestions (SurveyId,CustomerId,QuestionTypeId,QuestionText,IsDeleted, QuestionNo)
	SELECT 
		@SurveyId, 1 AS CustomerId, QuestionTypeId, 'Who sang this song ? Song : "Everything I  do"' AS QuestionText,
		1 AS IsDeleted, 7
	FROM MS_QuestionTypes WHERE QuestionCode = 'TextInput'
	
	SET @DefaultAns = @@IDENTITY
	...
	SELECT @Q7, SettingId, 
			CASE WHEN SettingId IN(1,2,3,13,14) THEN 'True'  
				...
				WHEN SettingId IN(7) THEN '0'
				WHEN SettingId IN(12) THEN '1'
				WHEN SettingId IN(8,9) THEN '' 
				WHEN SettingId IN(15) THEN 'Bryan Adams' END AS Value
	FROM MS_QuestionSettings
