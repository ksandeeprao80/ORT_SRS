-- Survey Id = @SurveyId 1.9, 4.1,5.1,6.1
BEGIN
BEGIN TRY
BEGIN TRAN
	...
	SELECT 
		'Seattle KLCK – By Invite December 2012' AS SurveyName, 1 AS CustomerId, 1 AS StarMarked, 1 AS RewardEnabled,
...
--------------------------------------------------------------------
	...
	(SurveyId,CustomerId,QuestionTypeId,QuestionText,IsDeleted, QuestionNo)
	SELECT @SurveyId, 1 AS CustomerId, QuestionTypeId, 'What is your exact age?' AS QuestionText, 1 AS IsDeleted, 7
	...
	SELECT @AgeQuestion, SettingId, 
		CASE WHEN SettingId IN(1,2,3,11,13) THEN 'True'  
			WHEN SettingId IN(4,5,6,10,14,16) THEN 'False'
			WHEN SettingId IN(7) THEN '0'
...
---3-4------------------------------------------------------------------------------------------------------------
	...
	(SurveyId,CustomerId,QuestionTypeId,QuestionText,IsDeleted, QuestionNo)
	SELECT @SurveyId, 1 AS CustomerId, QuestionTypeId, 'Gender:' AS QuestionText, 1 AS IsDeleted, 9 
	FROM MS_QuestionTypes WHERE QuestionCode = 'SingleChoice'
	...
	SELECT @QuestionId1, SettingId, 
		CASE WHEN SettingId IN(1,2,3,10,11,13) THEN 'True'  
			WHEN SettingId IN(4,5,6,14,16) THEN 'False'
			WHEN SettingId IN(7) THEN '0'
...
---5-6------------------------------------------------------------------------------------------------------------
	...
	INSERT INTO DBO.TR_SurveyQuestions
 	(SurveyId,CustomerId,QuestionTypeId,QuestionText,IsDeleted, QuestionNo)
	SELECT @SurveyId, 1 AS CustomerId, QuestionTypeId, 'Ethnic Background' AS QuestionText, 1 AS IsDeleted, 11 
	FROM MS_QuestionTypes WHERE QuestionCode = 'SingleChoice'
...
---------------------------------------------------------------------------------------------------------------		
	...
	-- Montage song value taking from file library
	DECLARE @SongId1 VARCHAR(12)
	SELECT @SongId1 = CONVERT(VARCHAR(12),FileLibId) FROM TR_FileLibrary 
	WHERE FileLibName = 'Montage Of Song Clips' AND [FileName] = '0812 KLCK Pod.mp3'
	...
	SELECT @QuestionId6, SettingId, 
			CASE WHEN SettingId IN(1,2,3,4,6,10,11,13,14,16) THEN 'False'  
				...
				WHEN SettingId IN(9) THEN @SongId1 END AS Value --703
	FROM MS_QuestionSettings
...
---------------------------------------------------------------------------------------------------------------
	...
	SELECT @QuestionId16, 'Don''t Listen / Don''t Know','Don''t Listen / Don''t Know'
 	...
 	INSERT INTO DBO.TR_SurveyQuestions
 	(SurveyId,CustomerId,QuestionTypeId,QuestionText,IsDeleted, QuestionNo)
	SELECT @SurveyId, 1 AS CustomerId, QuestionTypeId, 'PageBreak' AS QuestionText, 1 AS IsDeleted, 41
	FROM MS_QuestionTypes WHERE QuestionCode = 'PageBreak'
...
---------------------------------------------------------------------------------------------------------------
	...
	INSERT INTO DBO.TR_SurveyQuestions
 	(SurveyId,CustomerId,QuestionTypeId,QuestionText,IsDeleted, QuestionNo)
	SELECT @SurveyId, 1 AS CustomerId, QuestionTypeId, 'PageBreak' AS QuestionText, 1 AS IsDeleted, 48
	FROM MS_QuestionTypes WHERE QuestionCode = 'PageBreak'
...
---------------------------------------------------------------------------------------------------------------
	...
	DECLARE @Question46 INT
 	...
 	SET @Question46  = @@IDENTITY
  	...
  	INSERT INTO DBO.TR_SurveyQuestions
 	(SurveyId,CustomerId,QuestionTypeId,QuestionText,IsDeleted, QuestionNo)
 	SELECT @SurveyId, 1 AS CustomerId, QuestionTypeId, 'PageBreak' AS QuestionText,1 AS IsDeleted, 50
 	FROM MS_QuestionTypes WHERE QuestionCode = 'PageBreak'
 ...
 ----------------------------------------------
 	...
 	INSERT INTO DBO.TR_SurveyQuestions
	(SurveyId,CustomerId,QuestionTypeId,QuestionText,IsDeleted, QuestionNo)
	SELECT @SurveyId, 1 AS CustomerId, QuestionTypeId, 'PageBreak' AS QuestionText,1 AS IsDeleted, 52
	FROM MS_QuestionTypes WHERE QuestionCode = 'PageBreak'
	
	-- Montage song value taking from file library
	DECLARE @SongId2 VARCHAR(12)
	SELECT @SongId2 = CONVERT(VARCHAR(12),FileLibId) FROM TR_FileLibrary 
	WHERE FileLibName = 'Montage Of Song Clips' AND [FileName] = '0312 POP CHR -1 montage.mp3'
	...
	SELECT @QuestionId20, SettingId, 
		CASE WHEN SettingId IN(1,2,3,4,6,10,11,13,14,16) THEN 'False'  
			...
			WHEN SettingId IN(7) THEN '0'
			WHEN SettingId IN(8,12,15) THEN '' 
			WHEN SettingId IN(9) THEN @SongId2 END AS Value --703
	FROM MS_QuestionSettings
 ...
 --34-----------------------------------------------
 	...
 	INSERT INTO DBO.TR_SurveyQuestions
 	(SurveyId,CustomerId,QuestionTypeId,QuestionText,IsDeleted, QuestionNo)
 	SELECT @SurveyId, 1 AS CustomerId, QuestionTypeId, 'PageBreak' AS QuestionText,1 AS IsDeleted, 54
 	FROM MS_QuestionTypes WHERE QuestionCode = 'PageBreak'
 ...
 --36-----------------------------------------------
 	...
 	INSERT INTO DBO.TR_SurveyQuestions
	(SurveyId,CustomerId,QuestionTypeId,QuestionText,IsDeleted, QuestionNo)
	SELECT @SurveyId, 1 AS CustomerId, QuestionTypeId, 'PageBreak' AS QuestionText,1 AS IsDeleted, 56
	FROM MS_QuestionTypes WHERE QuestionCode = 'PageBreak'
    
    -- Montage song value taking from file library
	DECLARE @SongId3 VARCHAR(12)
	SELECT @SongId3 = CONVERT(VARCHAR(12),FileLibId) FROM TR_FileLibrary 
	WHERE FileLibName = 'Montage Of Song Clips' AND [FileName] = '0312 QM Montage Version #1.mp3'
	...
	SELECT @QuestionId22, SettingId, 
		CASE WHEN SettingId IN(1,2,3,4,6,10,11,13,14,16) THEN 'False'  
			WHEN SettingId IN(5) THEN 'True'
				WHEN SettingId IN(7) THEN '0'
				WHEN SettingId IN(8,12,15) THEN '' 
				WHEN SettingId IN(9) THEN @SongId3 END AS Value --703
	FROM MS_QuestionSettings
 --37-----------------------------------------------
 	...
 	INSERT INTO DBO.TR_SurveyQuestions
 	(SurveyId,CustomerId,QuestionTypeId,QuestionText,IsDeleted, QuestionNo)
 	SELECT @SurveyId, 1 AS CustomerId, QuestionTypeId, 'PageBreak' AS QuestionText,1 AS IsDeleted, 58
 	FROM MS_QuestionTypes WHERE QuestionCode = 'PageBreak'
  ...
  --39-----------------------------------------------
 	...
 	INSERT INTO DBO.TR_SurveyQuestions
	(SurveyId,CustomerId,QuestionTypeId,QuestionText,IsDeleted, QuestionNo)
	SELECT @SurveyId, 1 AS CustomerId, QuestionTypeId, 'PageBreak' AS QuestionText,1 AS IsDeleted, 60
	FROM MS_QuestionTypes WHERE QuestionCode = 'PageBreak'
	
	-- Montage song value taking from file library
	DECLARE @SongId4 VARCHAR(12)
	SELECT @SongId4 = CONVERT(VARCHAR(12),FileLibId) FROM TR_FileLibrary 
	WHERE FileLibName = 'Montage Of Song Clips' AND [FileName] = '0312 RHYTHM CHR -2 montage.mp3'
	...
	SELECT @QuestionId24, SettingId, 
		CASE WHEN SettingId IN(1,2,3,4,6,10,11,13,14,16) THEN 'False'  
			...
			WHEN SettingId IN(7) THEN '0'
			WHEN SettingId IN(8,12,15) THEN '' 
			WHEN SettingId IN(9) THEN @SongId4 END AS Value--703
	FROM MS_QuestionSettings
 ...
 --40-----------------------------------------------
 	...
 	INSERT INTO DBO.TR_SurveyQuestions
 	(SurveyId,CustomerId,QuestionTypeId,QuestionText,IsDeleted, QuestionNo)
 	SELECT @SurveyId, 1 AS CustomerId, QuestionTypeId, 'PageBreak' AS QuestionText,1 AS IsDeleted, 62
 	FROM MS_QuestionTypes WHERE QuestionCode = 'PageBreak'
...
---------------------------------------------------------------------------------------------------------------
	...
	INSERT INTO DBO.TR_SurveyQuestions
	(SurveyId,CustomerId,QuestionTypeId,QuestionText,IsDeleted, QuestionNo)
	SELECT @SurveyId, 1 AS CustomerId, QuestionTypeId,	'Sorry you did not qualify for survey, better luck next time.', 1,64
...
---------------------------------------------------------------------------------------------------------------
	...
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT @SurveyId,SettingId,1,'False' FROM MS_SurveySettings WHERE SettingName = 'ALLOW_ANONYMOUS'
	...
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT @SurveyId,SettingId,1,'10' FROM MS_SurveySettings WHERE SettingName = 'TestListRandomNo'
	...
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'False' FROM MS_SurveySettings WHERE SettingName = 'CheckQuota'
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'0' FROM MS_SurveySettings WHERE SettingName = 'QuotaCount' 
	 --Quota Email
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'False' FROM MS_SurveySettings WHERE SettingName = 'SendQuotaFullEmail' 
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'' FROM MS_SurveySettings WHERE SettingName = 'QuotaFullToEmailId'
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'' FROM MS_SurveySettings WHERE SettingName = 'QuotaFullMessageId'
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'' FROM MS_SurveySettings WHERE SettingName = 'E_O_S_MESSAGE_TEXT'
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'' FROM MS_SurveySettings WHERE SettingName = 'E_O_S_EMAIL_TEXT'
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'dd-mm-yyyy' FROM MS_SurveySettings WHERE SettingName = 'DateTimeFormat'
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'False' FROM MS_SurveySettings WHERE SettingName = 'RewardApplicable'
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'True' FROM MS_SurveySettings WHERE SettingName = 'VerifySkipLogic'
	INSERT INTO TR_SurveySettings(SurveyId,SettingId,CustomerId,Value) SELECT  @SurveyId,SettingId,1,'1' FROM MS_SurveySettings WHERE SettingName = 'TemplateId'
	...
	-- Following table
	DECLARE @Hal INT, @Merrie INT
	SELECT @Hal = PanelistId FROM MS_PanelMembers WHERE PanelistName = 'Hal Rood'
	SELECT @Merrie = PanelistId FROM MS_PanelMembers WHERE PanelistName = 'Merrie Farley'
	
	INSERT INTO MS_Invite
	(SurveyId,InviteType,InvitePassword,ToBeSentDate,ExternalPanelEmailId,InternalPanelId)
	SELECT @SurveyId,1,NULL,GETDATE(),NULL, @Hal UNION
	SELECT @SurveyId,1,NULL,GETDATE(),NULL, @Merrie 
	-------------------------------------------------------------------------------------
	...
	INSERT INTO TR_SkipLogic(LogicExpression,TrueAction,FalseAction,SurveyId) -- Montage
	...
	EXEC DBO.UspPublishSurvey @SurveyId
	---------
