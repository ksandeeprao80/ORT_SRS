...
BEGIN TRAN
	...
	SELECT 
		'Kishore Kumar Music Survey' AS SurveyName, 1 AS CustomerId, 1 AS StarMarked, 1 AS RewardEnabled,
		1 AS CreatedBy, GETDATE() AS CreatedDate, 1 AS InActive, 1 AS StatusId,
		1 AS CategoryId, 1 AS LanguageId, GETDATE()+17
...
-- 	SELECT @SurveyId,SettingId,1,'True' FROM MS_SurveySettings 
	...
	SET TS.Value = CASE WHEN MS.SettingName = 'SurveyTemplateName' THEN 'surveyEngine1.html'
					...
					WHEN MS.SettingName = 'SurveyWelcomePage' THEN 'surveyEngine1_welcome.html'  
	...
	WHERE MS.SettingName IN('SurveyTemplateName','SurveyThankuPage','SurveyWelcomePage','AutoAdvance')

	INSERT INTO TR_SurveySettings
	(SurveyId, SettingId,CustomerId,Value)
	SELECT 
		@SurveyId,SettingId,1,
		CASE WHEN SettingName = 'ANSWER_FONT' THEN 'Tahoma'
			WHEN SettingName = 'ANSWER_FONT_COLOR' THEN '#C71585'
			WHEN SettingName = 'ANSWER_FONT_SIZE' THEN '13px'
			WHEN SettingName = 'HEADER_FONT' THEN 'Tahoma'
			WHEN SettingName = 'HEADER_FONT_SIZE' THEN '25px'
			WHEN SettingName = 'HEADER_FONT_COLOR' THEN '#DA70D6' 
			WHEN SettingName = 'QUESTION_FONT' THEN 'Tahoma'
			WHEN SettingName = 'QUESTION_FONT_COLOR' THEN '#CB97FF'  	
			WHEN SettingName = 'QUESTION_FONT_SIZE' THEN '15px' END AS Value
	FROM MS_SurveySettings WHERE SettingName LIKE '%FONT%'

	--SELECT * FROM MS_SurveySettings WHERE SettingName LIKE '%FONT%'
	--SELECT * FROM TR_SurveySettings WHERE SurveyId = 1094
	-- UPDATE TR_SurveySettings SET Value = '#C71585' WHERE SurveyId = 1094 AND SettingId = 9 -- ans
	-- UPDATE TR_SurveySettings SET Value = '#DA70D6' WHERE SurveyId = 1094 AND SettingId = 33 -- hea
	-- UPDATE TR_SurveySettings SET Value = '#CB97FF' WHERE SurveyId = 1094 AND SettingId = 24 -- que
