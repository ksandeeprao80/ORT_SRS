IF EXISTS 
(
	SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID('[dbo].[UspGetFileLibraryList]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
DROP PROCEDURE [dbo].[UspGetFileLibraryList]

GO
/*
EXEC UspGetFileLibraryList 1,NULL,'<?xml version="1.0" encoding="utf-16"?>
<User>
	<UserId>5</UserId>
...
*/
CREATE PROCEDURE DBO.UspGetFileLibraryList
	@CustomerId INT,
	@LibId INT = NULL,
...
BEGIN TRY
	...
	BEGIN
		SELECT  
			TFL.FileLibId, TFL.LibId AS LibraryId, 
			...
			TL.CustomerId, TL.LibName, ISNULL(TFL1.NoOfSongs,0) AS NoOfSongs
		...
		LEFT OUTER JOIN
		(
			SELECT TFL.LibId, COUNT(1) AS NoOfSongs
			FROM DBO.TR_FileLibrary TFL
			INNER JOIN TR_Library TL
				ON TFL.LibId = TL.LibId
