IF EXISTS 
(
	SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID('[dbo].[UspGetEditSurveySettings]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
DROP PROCEDURE [dbo].[UspGetEditSurveySettings]

GO
-- EXEC UspGetEditSurveySettings NULL,1602

CREATE PROCEDURE DBO.UspGetEditSurveySettings
	@SettingId INT = NULL,
	@SurveyId INT = NULL
AS
SET NOCOUNT ON
...
BEGIN TRY
	...
	IF EXISTS
	(
		SELECT 1 FROM DBO.MS_SurveySettings MSS INNER JOIN DBO.TR_SurveySettings TSS 
	...
	BEGIN
		IF EXISTS
		(
			SELECT 1 FROM DBO.TR_SurveySettings TSS INNER JOIN DBO.MS_SurveySettings MSS
				ON MSS.SettingId = TSS.SettingId AND TSS.SurveyId = @SurveyId
		...
		BEGIN
			...
			FROM DBO.TR_SurveySettings TSS INNER JOIN DBO.MS_SurveySettings MSS
				ON MSS.SettingId = TSS.SettingId AND TSS.SurveyId = @SurveyId
		...
		BEGIN	
			...
			INNER JOIN 
			(
				SELECT TSS.* FROM DBO.TR_SurveySettings TSS
	...
	BEGIN
		...
		BEGIN
			...
			INNER JOIN 
			(
				SELECT TSS.* FROM DBO.TR_SurveySettings TSS INNER JOIN DBO.MS_SurveySettings MSS
					ON MSS.SettingId = TSS.SettingId AND TSS.SurveyId = @SurveyId
