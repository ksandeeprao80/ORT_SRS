IF EXISTS 
(
	SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID('[dbo].[UspGetGraphicLibrary]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
DROP PROCEDURE [dbo].[UspGetGraphicLibrary]

GO
/*
-- EXEC UspGetGraphicLibrary 1,'<?xml version="1.0" encoding="utf-16"?>
<User>
	<UserId>5</UserId>
...
*/
CREATE PROCEDURE DBO.UspGetGraphicLibrary
	@CustomerId INT,
	@LibTypeId INT,
...
BEGIN TRY
	...
	BEGIN
		SELECT  
			TL.LibId, TL.LibTypeId, TL.LibName, TL.CustomerId, TL.IsActive, TL.CreatedBy, 
			TL.CreatedOn, TL.ModifiedBy, TL.ModifiedOn
		FROM TR_Library TL
		INNER JOIN @UserInfo UI
			ON TL.CustomerId = (CASE WHEN UI.RoleDesc = 'SA' THEN TL.CustomerId ELSE CONVERT(INT,UI.CustomerId) END)
		WHERE TL.LibTypeId = @LibTypeId
			AND TL.IsActive = 1
	END
	ELSE
	BEGIN
