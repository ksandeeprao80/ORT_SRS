IF EXISTS 
(
	SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID('[dbo].[UspSaveInviteReminderLog]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
DROP PROCEDURE [dbo].[UspSaveInviteReminderLog]

GO
/*
EXEC UspSaveInviteReminderLog '<?xml version="1.0" encoding="utf-16"?>
<ArrayOfMailData>
  <MailData>
...
CREATE PROCEDURE DBO.UspSaveInviteReminderLog
	@XMlData AS NTEXT
AS
SET NOCOUNT ON
BEGIN
BEGIN TRY
	...
	SELECT
		...
		Child.Elm.value('(IsSuccess)[1]','VARCHAR(50)') AS IsSuccess,
	...
	SELECT 
		...
		PanelMemberEmailId, InviteSubject, InviteMessage, CONVERT(INT,RowId), @CurrentDate, LEFT(IsSuccess,1), 
		ErrorMessage
	FROM #InviteLog
	...
	IF @RowId <> 0 
	BEGIN	
		IF EXISTS(SELECT 1 FROM #InviteLog WHERE LEFT(LTRIM(RTRIM(IsSuccess)),1) = 'Y')
		BEGIN
			UPDATE TID
			SET TID.ReminderSentDate = @CurrentDate,
				TID.Locked = NULL
			FROM DBO.TR_InviteDetails TID
			INNER JOIN #InviteLog IL
				ON TID.RowId = CONVERT(INT,IL.RowId)
				AND LEFT(LTRIM(RTRIM(IL.IsSuccess)),1) = 'Y'
			
			SELECT 1 AS RetValue, 'Successfully Sent Date Updated' AS Remark
