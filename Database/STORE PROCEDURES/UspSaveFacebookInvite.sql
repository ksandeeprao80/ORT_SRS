IF EXISTS 
(
	SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID('[dbo].[UspSaveFacebookInvite]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
DROP PROCEDURE [dbo].[UspSaveFacebookInvite]

GO
/*
EXEC UspSaveFacebookInvite '<?xml version="1.0" encoding="utf-16"?>
<SurveyInvite>
  <Survey>
...
CREATE PROCEDURE DBO.UspSaveFacebookInvite
	@XMlData AS NTEXT
AS
SET NOCOUNT ON
BEGIN
BEGIN TRY
	...
	SELECT 	
		CONVERT(INT,SI.SurveyId), MIT.InviteType, NULL, GETDATE(), NULL, CONVERT(INT,SI.InvitedPanelId), GETDATE()
	...
	BEGIN
		...
		SELECT 
			@InviteId, SI.SocialInviteTitle, SI.SocialInviteDescription, 2 AS ChannelId
		FROM #SurveyInvite1 SI
		--INNER JOIN DBO.MS_Channel MC
		--	ON LTRIM(RTRIM(SI.SocialNetworkType)) = LTRIM(RTRIM(MC.ChannelDescription))
		--WHERE ISNULL(SI.SocialInviteTitle,'') <> '' 
		--	AND ISNULL(SI.SocialInviteDescription,'') <> ''
	END
