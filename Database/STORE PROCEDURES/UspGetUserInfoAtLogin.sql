IF EXISTS 
(
	SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID('[dbo].[UspGetUserInfoAtLogin]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
DROP PROCEDURE [dbo].[UspGetUserInfoAtLogin]

GO  
-- EXEC UspGetUserInfoAtLogin 'SRS007'
CREATE PROCEDURE DBO.UspGetUserInfoAtLogin
	@LoginId VARCHAR(30)
AS
SET NOCOUNT ON
...
BEGIN TRY
	...
	SELECT 
		...
		MTZ.TimeZoneId, ISNULL(MTZ.TimeZone,'') AS TimeZone, ISNULL(MU.Phone1,'') AS Phone1, ISNULL(MU.Phone2,'') AS Phone2, 
		ISNULL(MU.Phone3,'') AS Phone3, ISNULL(MU.UserCode,'') AS UserCode, MU.LoginId, ISNULL(MU.Department,'') AS Department,
		MU.CreatedBy, CONVERT(VARCHAR(10),MU.CreatedOn,121) AS CreatedOn, ISNULL(MU.ModifiedBy,0) AS ModifiedBy,
		CASE WHEN ISNULL(MU.ModifiedOn,'') <> '' THEN CONVERT(VARCHAR(25),MU.ModifiedOn,121) ELSE '' END AS ModifiedOn,
		MR.Hierarchy, MR.OpenAccess, MU.PasswordRequested, MU.LinkKey 
	FROM DBO.MS_Users MU 
	INNER JOIN DBO.MS_Roles MR
		ON MU.UserType = MR.RoleType	
		AND LTRIM(RTRIM(MU.LoginId)) = LTRIM(RTRIM(@LoginId))
		AND MU.IsActive = 1
