IF EXISTS 
(
	SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID('[dbo].[UspGetSurveyLibrary]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
DROP PROCEDURE [dbo].[UspGetSurveyLibrary]

GO
/*
-- EXEC UspGetSurveyLibrary 1,'<?xml version="1.0" encoding="utf-16"?>
<User>
	<UserId>5</UserId>
...
*/
CREATE PROCEDURE DBO.UspGetSurveyLibrary
	@CustomerId INT,
...
BEGIN TRY
	...
	BEGIN
		...
		SELECT  
			TSL.SurveyLibId, TSL.LibId AS LibraryId, TSL.SurveyId, 
			LTRIM(RTRIM(ISNULL(TSL.SurveyLibName,''))) AS SurveyLibName,
			LTRIM(RTRIM(ISNULL(TSL.Category,''))) AS Category, 
			LTRIM(RTRIM(ISNULL(TL.LibName,''))) AS LibName,
			ISNULL(TL.CustomerId,0) AS CustomerId, TSL.IsActive,
			TSL.CreatedBy, TSL.CreatedOn, TSL.ModifiedBy, TSL.ModifiedOn
