IF EXISTS 
(
	SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID('[dbo].[UspGetMySurveyPagination]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
DROP PROCEDURE [dbo].[UspGetMySurveyPagination]

GO
/*
-- UspGetMySurveyPagination 4,'<?xml version="1.0" encoding="utf-16"?>
<User>
  <UserId>5</UserId>
...
*/
CREATE PROCEDURE DBO.UspGetMySurveyPagination
	@CustomerId INT,
...
BEGIN TRY
	...
	(
		SurveyId INT, SurveyName VARCHAR(50), CustomerId INT, Responses INT, RequiredSamples INT,
	...
	INNER JOIN DBO.MS_SurveyStatus MSS
		ON TS.StatusId = MSS.StatusId AND TS.IsActive = 1
	...
	(
		...
		(	
			...
			INNER JOIN DBO.TR_SurveyQuestions TSQ
				ON TR.QuestionId = TSQ.QuestionId AND TSQ.IsDeleted = 1 AND TR.[Status] = 'C'
			INNER JOIN @UserInfo UI
		...
		ON TS.SurveyId = TR1.SurveyId AND TS.CustomerId = TR1.CustomerId
	...
	(
		RowNum INT IDENTITY NOT NULL PRIMARY KEY CLUSTERED, SurveyId INT NOT NULL, SurveyName VARCHAR(50), 
	...
	SELECT MS.* FROM #MySurvey MS
	INNER JOIN @OrderedKeys OK ON OK.SurveyId = MS.SurveyId
