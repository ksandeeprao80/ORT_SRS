IF EXISTS 
(
	SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID('[dbo].[UspRptCrossTabDataRanker]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
DROP PROCEDURE [dbo].UspRptCrossTabDataRanker

GO

-- EXEC UspRptCrossTabDataRanker 595,NULL,NULL,NULL
-- EXEC UspRptCrossTabDataRanker 596,2012

CREATE PROCEDURE DBO.UspRptCrossTabDataRanker 
	@ReportId INT,
	@SurveyId INT = NULL,
	@SongId INT = NULL,
...
BEGIN TRY
	...
	(
		...
		(
			...
			UNION ALL
			SELECT SurveyId, 1 AS IsBaseSurvey FROM DBO.TR_Trends WITH(NOLOCK) WHERE ReportId = @ReportId AND SurveyId IS NOT NULL
		) A
	) CS 
	...
	CREATE TABLE #TR_TrendCrossTabsRanker
	(
	...
	INSERT INTO #TR_TrendCrossTabsRanker
	(
	...
	WHERE TTCT.ReportId = @ReportId AND TTCT.BaseSurveyId = @SurveyId
	...
	(	
		...
		FROM #TR_TrendCrossTabsRanker TTCT 
...
-- 0
	...
	(
		SELECT 
			...
			S.QuestionText
		FROM #StableSet S
		INNER JOIN #TR_TrendCrossTabsRanker TC 
...
-- 1		
	...
	INNER JOIN #TR_TrendCrossTabsRanker TTCT 
	...
	SELECT 
		...
		ISNULL(TTCT.OptionDisplayText,REPLACE(TTCT.MTBOptionName,'-',':')) AS MTBOptionName,
...
-- 3	
	...
	(
		FilterAnswer NVARCHAR(500), Conjuction VARCHAR(20), FilterName NVARCHAR(100), FilterId INT, 
	...
	(
		FilterAnswer, Conjuction, FilterName, FilterId, Operator, QuestionId, 
...
-- 8	
	...
	--DROP TABLE #TR_TrendCrossTabsRanker
