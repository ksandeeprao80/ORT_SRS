IF EXISTS 
(
	SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID('[dbo].[UspGetQuestionLibrary]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
DROP PROCEDURE [dbo].[UspGetQuestionLibrary]

GO
/*
-- EXEC UspGetQuestionLibrary 1,'<?xml version="1.0" encoding="utf-16"?>
<User>
	<UserId>5</UserId>
...
*/
CREATE PROCEDURE DBO.UspGetQuestionLibrary
	@CustomerId INT,
...
BEGIN TRY
	...
	BEGIN
		SELECT  
			TQL.QuestionLibId, TQL.LibId AS LibraryId, TQL.QuestionId, 
			LTRIM(RTRIM(ISNULL(TQL.QuestionLibName,''))) AS QuestionLibName,
			LTRIM(RTRIM(ISNULL(TQL.Category,''))) AS Category, 
			LTRIM(RTRIM(ISNULL(TL.LibName,''))) AS LibName,
			TL.CustomerId, TQL.IsActive, TQL.CreatedBy, TQL.CreatedOn, 
			TQL.ModifiedBy, TQL.ModifiedOn, 
