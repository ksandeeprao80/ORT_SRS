IF EXISTS 
(
	SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID('[dbo].[UspGetCompletedSurveys]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
DROP PROCEDURE [dbo].UspGetCompletedSurveys

GO
/*
EXEC UspGetCompletedSurveys @CustomerId='67',@PageData='<?xml version="1.0" encoding="utf-16"?>
<PageModel>
  <Page>1</Page>
  <Start>0</Start>
  <Limit>25</Limit>
  <SortBy />
</PageModel>',@XmlUserInfo='<?xml version="1.0" encoding="utf-16"?>
<User>
  <UserId>102</UserId>
  <UserName>QASRSAdm</UserName>
  <UserDetails>
...
*/
CREATE PROCEDURE DBO.UspGetCompletedSurveys
	@CustomerId INT,
	...
	BEGIN
		...
		BEGIN
			...
			(
				...
				CategoryId, LanguageId, LangauageName, SurveyEndDate, TotalRewards
			)		
			SELECT 
			...
			INNER JOIN DBO.TR_Reward TRW
				ON TS.SurveyId = TRW.SurveyId 
			...
			WHERE ISNULL(TRW.TotalRewards,'0') <> '0' 	
		END
		...
		BEGIN
			...
			INNER JOIN @UserInfo UI
				ON TS.CustomerId = CONVERT(INT,UI.CustomerId)  
				AND TS.CreatedBy = (CASE WHEN UI.RoleDesc = 'GU' THEN TS.CreatedBy ELSE CONVERT(INT,UI.UserId) END)  	
			INNER JOIN
			(
				SELECT 
					COUNT(RCNT.Responses) AS Responses, RCNT.SurveyId, RCNT.CustomerId	 
				...
				(	
					...
					INNER JOIN DBO.TR_SurveyQuestions TSQ
						...
						AND TSQ.IsDeleted = 1 AND TR.[Status] = 'C'
					INNER JOIN @UserInfo UI
						ON TSQ.CustomerId = CONVERT(INT,UI.CustomerId)  
					GROUP BY TSQ.SurveyId, TSQ.CustomerId, TR.RespondentId, TR.SessionId
				) RCNT
				GROUP BY RCNT.SurveyId, RCNT.CustomerId
			) TR1
		...
		(
			...
			SurveyEndDate VARCHAR(10), TotalRewards VARCHAR(50)
		)
		
		SET ROWCOUNT @LastRecord
		
		DECLARE @SqlScript NVARCHAR(4000)
		SET @SqlScript = 'SELECT DISTINCT SurveyId, SurveyName, CustomerId, Responses, RequiredSamples, StarMarked, RewardEnabled, 
			CreatedBy, CreatedDate, ModifiedBy, ModifiedDate, IsActive, StatusId, SurveyStatusName, 
			CategoryId, LanguageId, LangauageName, SurveyEndDate, TotalRewards  
