IF EXISTS 
(
	SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID('[dbo].[UspSearchLibraryMessage]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
DROP PROCEDURE [dbo].[UspSearchLibraryMessage]

GO
/*
--EXEC UspSearchLibraryMessage 'undefined',11,'<?xml version="1.0" encoding="utf-16"?>
<User>
	<UserId>5</UserId>
...
*/ 
CREATE PROCEDURE DBO.UspSearchLibraryMessage
	@LibId VARCHAR(20),
	@CategoryId INT,
...
BEGIN TRY
	...
	-- Condition applied due to @LibId sometime comes as 'Undefined' 
	...
	ELSE
	BEGIN
		SET @LibraryId = CONVERT(INT,@LibId)
	END
	...
	BEGIN	
		...
		(
			...
			WHERE TLC.LibId = @LibraryId
				AND TLC.CategoryId = @CategoryId
		)
		BEGIN
			SELECT   
				TL.LibId, TML.MessageLibId, TML.MessageDescription, TL.CustomerId, TL.LibName, TML.MessageText, 
				TLC.CategoryId, TLC.CategoryName, MMT.MessageTypeId, MMT.MessageTypeName, 
				ISNULL(TML.TemplateName,'') AS TemplateName, 1 AS RetValue,
				TML.IsActive, TML.CategoryId, TML.CreatedBy, TML.CreatedOn, TML.ModifiedBy, TML.ModifiedOn
			FROM DBO.TR_MessageLibrary TML 
			INNER JOIN DBO.MS_MessageType MMT 
				ON MMT.MessageTypeId = TML.MessageTypeId
			INNER JOIN DBO.TR_Library TL 
				ON TML.LibId = TL.LibId
				AND TML.LibId = @LibraryId
				AND TML.IsActive = 1
			...
			INNER JOIN DBO.TR_LibraryCategory TLC
				...
				AND TLC.CategoryId = TML.CategoryId
				AND TLC.CategoryId = ISNULL(@CategoryId,TLC.CategoryId)
		END
		ELSE
		BEGIN
