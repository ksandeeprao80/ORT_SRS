IF EXISTS 
(
	SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID('[dbo].[UspGetPanelMembers]') 
	AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
DROP PROCEDURE [dbo].[UspGetPanelMembers]

GO
/*
-- EXEC UspGetPanelMembers 1,1,'<?xml version="1.0" encoding="utf-16"?>
<User>
	<UserId>5</UserId>
...
BEGIN TRY
	...
	BEGIN
		SELECT 
			MPM.PanelistId, LTRIM(RTRIM(MPM.PanelistName)) AS PanelistName, MPM.CustomerId, 
			MPM.LibId, MPM.LastUsed, MPM.IsActive, ISNULL(TPC.CategoryId,0) AS CategoryId,   
			ISNULL(MR.Members,0) AS Members
		FROM DBO.MS_PanelMembers MPM WITH(NOLOCK)
		INNER JOIN DBO.TR_Library TL
			ON MPM.LibId = TL.LibId
			AND MPM.PanelistId = ISNULL(@PanelistId,MPM.PanelistId)
		...
		--WHERE MPM.PanelistId = ISNULL(@PanelistId,MPM.PanelistId)
 	END
	ELSE
	BEGIN
